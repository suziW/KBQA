...
SPARQL_COUNT_TEM = u"SELECT COUNT({select}) WHERE {{\n" + \
             u"{expression}\n" + \
             u"}}\n"

SPARQL_ASK_TEM = u"ASK {{\n" + \
             u"{expression}\n" + \
             u"}}\n"
...
class QuestionSet:
    ...
    def has_cooperation_question(word_objects):
        ...
        if person1 is not None and person2 is not None:
            ...
            return SPARQL_SELECT_TEM.format(select=select,
                                          expression=e)
        else:
            return None
    ...
    def has_quantity_question(word_objects):
        ...
        for w in word_objects:
            if w.pos == pos_person:
                ...
                sparql = SPARQL_COUNT_TEM.format( select=select, expression=e)
                break
    ...
    def is_comedian_question(word_objects):
        ...
        for w in word_objects:
            if w.pos == pos_person:
                ...
                sparql = SPARQL_ASK_TEM.format(expression=e)
                break
    ...
    def has_basic_person_info_question(word_objects):
        ...
        for w in word_objects:
            if w.pos == pos_person:
                ...
                sparql = SPARQL_SELECT_TEM.format( select=select, expression=e)
